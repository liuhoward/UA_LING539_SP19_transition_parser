...
def get_feature_row(stack: Sequence[Dep], queue: Sequence[Dep]) -> dict:
    ...
    if len(stack) >= 2:
        ...
        feature_row[f'stack_1212_form_upos={stack[-1].form.lower() + stack[-2].form.lower() + stack[-1].upos + stack[-2].upos}'] = 1
        feature_row[f'stack_1212_lemma_upos={stack[-1].lemma.lower() + stack[-2].lemma.lower() + stack[-1].upos + stack[-2].upos}'] = 1
    ...
    if len(stack) > 0 and len(queue) > 0:
        feature_row["stack_queue_form={}".format(stack[-1].form.lower() + queue[0].form.lower())] = 1
        feature_row["stack_queue_upos={}".format(stack[-1].upos + queue[0].upos)] = 1
        feature_row["stack_queue_lemma={}".format(stack[-1].lemma.lower() + queue[0].lemma.lower())] = 1
...
class Classifier:
    def __init__(self, parses: Iterator[Sequence[Dep]]):
        ...
        self.classifier = LogisticRegression(solver='lbfgs', penalty='l2', tol=1e-3, max_iter=150, multi_class='multinomial')
